    ...
    for step in steps:
        if step.strip():
            ...
            if requirement_match and content_match:
                ...
                print(f"[WARN] Skipped step due to missing requirement/content:\n{step}\n")
...
def get_response(model_name, prompt):
    ...
    print("Gemini loading {}...".format(model_name))
...
def NER_agent(questions, model_name="gemini-2.5-pro"):
    template = f"""
    Factual knowledge is information that aligns with objective reality and can be verified through evidence or observation, such as scientific facts or historical events.
...
def get_label(input_string,answer):
    planing = get_response("gemini-2.5-pro", planning(input_string,answer))
